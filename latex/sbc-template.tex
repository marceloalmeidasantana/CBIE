...
Este estudo tenta responder algumas dessas perguntas, a forma encontrada para responder essas perguntas foi utilizando dois métodos. A necessidade da utilização de dois métodos distintos se deu no sentido de poder fazer um coparativo entres os resultados obtidos a fim de chegar a conclusões mais sólidas. 

O primeiro método usado foi aplicar um questionário aos tutores online a fim de identificar se todos os recursos disponibilizados pelo moodle estão sendo utilizados e quais dificuldades encontradas por eles em usar algumas dessas ferramentas. O segundo método foi aplicar técnicas de mineração de dados sobre o AVA moodle, com o propósito de identificar quais as ferramentas mais usadas e se essas ferramentas estão influenciando no desempenho dos estudantes. Para isso, foi usada uma importante característica do moodle que é a de manter registros detalhados de todas as atividades que os alunos realizam, gerando grandes quantidades de dados. Toda esta informação gerada fornece uma ``mina de ouro'' de dados educacionais que podem ser usados para analisar a viabilidade dos recursos disponibilizados.
...
Para realização do estudo, foi nescessário auxilio de uma ferramenta para extração, transformação e carga de dados e outra para aplicar os algoritmos de mineração de dados. As ferramentas escolhidas foram Pentaho Data Integration \cite{Pentaho:2014} e Weka \cite{Weka:2014} ambos os softwares são livre.
...
%[comment] Você só apresentou 3 trabalhos relacionados, achei pouco. Não precisa apresentar somente estudos que utilizam EDM
...
%[comment] Pq os procedimento descritos abaixo foram utilizados para realizar a discretizacao? 
